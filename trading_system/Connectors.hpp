...
// an auxiliary function to convert string of fractional bond price to double of mid-price
double FractionalToPrice(string frac_price)
{
    ...
    //bond price = (1*S1+ (1/32)S2 + (1/32) * (1/32)S3) * 100
...
{
public:
    void Publish(Position<V> data)      // publish data to file
    {
        ofstream out("output/pos.txt", ios::app);
...
{
public:
    void Publish(PV01<V> data)       // publish data to file
    {
        ofstream out("output/risk.txt", ios::app);
        V product = data.GetProduct();
        out << product.GetProductId() << ", ";
        out << data.GetPV01() << endl;
        out.close();
    }
...
// HistoricalStreamingConnector class
template<typename V>
class HistoricalStreamingConnector : public Connector<PriceStream<V> >
{
public:
    void Publish(PriceStream<V> data)      // Publish data into the file
    {
...
// HistoricalExecutionConnector class
template<typename V>
class HistoricalExecutionConnector : public Connector<ExecutionOrder<V> >
{
public:
    void Publish(ExecutionOrder<V> data)        // Publish data into the file
    {
        ofstream out("output/execution.txt", ios::app);
        V product = data.GetProduct();
        out << product.GetProductId() << ", " << data.GetOrderId() << ", " <<
            data.GetPricingSide() << ", " << data.GetPrice() << ", " << data.GetVisibleQuantity() << endl;
        out.close();
    }
//    {
//        ofstream out("output/execution.txt", ios::app);
...
{
public:
    void Publish(Inquiry<V> data)       // publish data to file
    {
        ofstream out("output/inquiry.txt", ios::app);
        string state;
...
public:
    ...
    {
        ofstream out("output/gui-snapshot.txt", ios::app);
        // get current time
        ptime cur_time = microsec_clock::local_time();
        // get current best bid/ask price of the product(mid-price and spread)
        V product = data.GetProduct();
...
public:
    ...
    void Subscribe(string file_name)        // process given file
    {
        ...
        if (in.is_open()) 
        {
            ...
            while (getline(in, line))
            {
                ...
                while (getline(line_stream, seg, ','))      // pare string by ','
                ...
                double price = FractionalToPrice(line_seg[3]);
...
public:
    ...
    void Subscribe(string file_name)        // process the given file
    {
        ifstream in(file_name);
        int counter = 0;
        ptime cur_time;
        if (in.is_open())
        {
            ...
            while (getline(in, line))
            {
                ...
                vector<string> line_seg;
                stringstream line_stream(line);
                while (getline(line_stream, seg, ',')) // parse string by ','`
                    line_seg.push_back(seg);
            ...
            cur_time = microsec_clock::local_time();
            cout << cur_time << " Price data from " << file_name <<  " is processed.\n\n";
        }
        else
        {
            cur_time = microsec_clock::local_time();
    ...
    void Subscribe(string file_name)        // process the given file
    {
        ...
        if (in.is_open())
        {
            ...
            while (getline(in, line))
            {
                ...
                {
                    cur_time = microsec_clock::local_time();
                    cout << cur_time << "  " << "  Order book data processed for " << productID << ".\n";
                }
                V product = Bond(productID, CUSIP, g_tickers[productID], g_coupons[productID], g_dates[productID]);
            ...
            cur_time = microsec_clock::local_time();
            cout << cur_time << "  Order book data from "<<file_name<<" is processed.\n\n";
        }
        else
        {
            cur_time = microsec_clock::local_time();
